...
if __name__ == '__main__':
    ...
    # load agent
    net = load_model('../../models/test_student_h_vs_ab.pt', BasicCNN)
    print(net)
    dh = DeepHeuristic(value_network=net)

    # player 1
    player1 = AlphaBetaAgent(depth=1, player=1, heuristic=dh, use_tt=True, verbose=True)

    # player 2
    # player2 = AlphaBetaAgent(depth=3, player=2, heuristic=max_consecutive_squares_eval)
    player2 = AlphaBetaAgent(depth=1, player=2, heuristic=total_consecutive_squares_eval, use_tt=True, verbose=True)
